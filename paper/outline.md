...
- Paragraph plan:
  ...
  - P3: We test it with a pre-registered oracle-arm 2x2 factorial on a Unitree Go2 policy pair in Isaac: two policies (stand, walk) crossed with two fault families (motor degradation, observation corruption), unshielded versus oracle-shielded, 960 independent blocks, 5 independent process seeds.
  ...
  - P6: Contributions, stated as a measurement and a method, plus one sentence conceding the residual pre-onset control, the simulation-only scope, and the OUTCOME BOUNDARY up front so no reviewer discovers any of it late. The outcome is post-onset fall rate and only that: `task_complete` in the artifacts is defined as `~fell` (`reliability_closed_loop.py:1191`), so it is not an independent task metric and the study measures NO cost for abandoning locomotion. Write "the fallback reduces falls", never "the shield is better" or "safer".
...
- Paragraph plan:
  ...
  - 5.2 P2a: Two corrections to how that must be phrased, both found by adversarial audit and both to be stated rather than quietly avoided. (i) Do NOT write that the process-level interval is "the conservative one". It is wider for some quantities and NARROWER for others (stand_obs full 0.79x, walk_obs full 0.60x, walk_motor leak-free 0.56x of the block-level half-width), because with 4 degrees of freedom the sample SD of five process means can come out small. Write instead that it is the CORRECT level for a claim about new seeds, and note that the measured between-process variance component is essentially zero (one-way ANOVA over processes: ICC ~ 0 in three of four cells, largest F = 1.78, p = 0.135). (ii) Do NOT call the process-level result assumption-free. At n = 5 the sign test, the exact sign-flip randomization test and the exact Wilcoxon signed-rank all bottom out at p = 0.0625, which is the minimum attainable value at five paired units. The data are as extreme as the design permits, but no exact distribution-free test can reject at alpha = 0.05 with five processes. The interval is parametric, resting on approximate normality of the five process means, which Shapiro-Wilk does not contradict (p in [0.07, 1.00]). Say this in the paper; a reviewer who runs a sign test will find it in one minute.
  - 5.2 P2b: Multiplicity. 18 process-level intervals are reported (4 cells and 2 families and the interaction, each on the full sample and the leak-free subset, plus 4 pre-onset controls). Under Holm over all 18, all 14 substantive quantities still reject; the weakest is the leak-free stand_obs cell at raw p = 3.59e-3, Holm-adjusted 1.79e-2. The four negative controls correctly do not reject. Report the correction rather than waiting to be asked for it.
  - 5.2 P3: What the two extra seeds changed. Nothing in sign or gate outcome; the point estimates moved by at most 0.65 pp per cell; and the one interval that crossed zero at n = 3 no longer does. Also report the one thing that got WORSE: the onset-threshold predicate behind the Section 6 mechanism argument went from 12 of 12 pairs exceptionless to 18 of 20, with 4 mismatching blocks in 960 under the fitted threshold (a minimum-misclassification threshold puts it at 2, so 4 is the conservative statement and the one we make). Do not omit this to keep the story clean.
  ...
  - 5.3 P3: **DO NOT use task completion as a secondary outcome.** It is `1 - fall rate` by construction (`reliability_closed_loop.py:1191`: `task_complete = ~fell`), verified numerically: `u_task + u_fell = 1.0000` in every cell. Quoting it as corroboration would be quoting the primary outcome twice. Either rename the column in the result table to make the identity explicit or delete the row. The other candidate, `return_until_first_fall`, is also unusable in the observation cells because the injected observation noise feeds the reward terms (stand_obs median -1588.76 unshielded vs -26.93 oracle), so it measures the corruption, not the task. State in the paper that no task-performance cost was measured and that this bounds what "benefit" can mean.
  - 5.3 P4: Fall-rate context, which is the readable version of the pp effects (from `result_table_n5.md`): stand_motor unshielded 14.65 percent versus oracle 38.18; stand_obs 10.41 versus 0.51; walk_motor 52.76 versus 59.30; walk_obs 19.93 versus 2.78. These make the mechanism visible: in the observation cells the oracle arm almost never falls, because the fallback zeroes the commanded action and thereby severs the fault's only causal pathway.
...
- Paragraph plan:
  ...
  - P2: n=1 policy family, one simulator, one fallback design (a static stand pose implemented as a null-action controller). The flip is a statement about this fallback's recovery authority, not about fallbacks in general. Concede in the same paragraph the sharpest form of the objection: because the fallback zeroes the commanded action, it is immune to observation faults and dependent on actuators BY CONSTRUCTION, so the sign flip is close to a deduction from the fallback's definition. That is why the paper leads with the mechanism (C2) and presents the study as measuring how large the effect is and confirming it is not an artifact, rather than as discovering that it exists. Do not stage the sign flip as a surprise; a reviewer who sees the fallback's definition first will call it a tautology.
  - P2a: The five processes generalize over RNG streams only. A process re-seeds the environment, torch and numpy; it is not an independent policy, environment, fallback or fault model. Say "five independent process seeds", never "five independent replications of the system".
  - P2b: The outcome boundary, repeated here because it is a limitation and not only a definition: post-onset fall rate is the only outcome. No task-performance cost is measured, so the study cannot say what the fallback costs when it freezes a walking robot. `task_complete` in the artifacts is `~fell` and is not a second outcome.
  ...
  - P4: What would falsify or extend this: a fallback with actuator-independent authority under motor degradation, the same factorial on hardware, or the same factorial with a genuine task-performance outcome alongside the fall rate, which would price the freeze the observation cells currently get for free.
  - P5: Inferential scope. Five process seeds is enough for a parametric interval and not enough for an exact distribution-free one; every reported rejection is parametric. More processes, not more blocks, is the only way to change that.
