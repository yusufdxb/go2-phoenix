...
| N1 | NEGATIVE, must be reported: the pre-registered pre-onset negative control failed at cell level; 3 of 4 cells exclude zero by 0.65 to 1.76 pp | 6 | `combined_summary.json` -> `pooled_pre_onset_negative_control_cells`, `gate_checks` |
...
- JOB (one sentence): Report the failed pre-onset negative control in full, state what it does and does not license, and pre-commit to the interpretation before the reviewer forms their own.
...
- Paragraph plan:
  ...
  - P2: What it returned. Stand/motor -0.65 [-1.30, -0.07]; stand/obs +1.69 [+1.11, +2.34]; walk/obs -1.76 [-3.19, -0.33]; walk/motor +0.39 [-1.37, +2.15]. Three of four exclude zero. `gate_passed: false`.
  - P3: What pools and what does not. Family-level pre-onset effects balance (motor -0.13 [-1.07, +0.81]; obs -0.03 [-0.78, +0.75]), which is consistent with symmetric noise but does not override a fail-closed cell-level criterion that was frozen before the run.
  - P4: SUPERSEDED 2026-08-24. The noise-floor reading is DELETED, not sourced. The characterization returned a REAL TREATMENT LEAK (see NEGATIVE_CONTROL_ANALYSIS.md, commit 179d7f5): env.reset() returns an observation whose root linear velocity, angular velocity, and projected gravity are stale from the previous block's terminal state, which is exactly what the treatment sets. Block 0, which has no predecessor, is bit-identical across arms in all 12 process-cells and gives a negative control of exactly +0.000 pp; blocks 1 to 47 diverge in all 12. A noise floor would be nonzero in block 0. Do not write a noise-floor paragraph here.
  - P5: BRANCH (b) IS NOW ACTIVE. The effect estimates from study v1 are not clean and the study cannot go to ICRA in this form. The path forward is the design fix and re-run specified in NEGATIVE_CONTROL_ANALYSIS.md (reset_settle_ticks probe, then a 1.53 h re-run re-frozen as v2), not a rewrite of this paragraph. Supporting evidence that the finding is probably real but not provable from v1: on the leak-free block-0 subset all four signs hold and the interaction reproduces at +23.02 pp [+13.63, +32.47] against the frozen +19.24, but that is 48 env pairs from 16 envs in each of 3 blocks, so its bootstrap understates uncertainty. It is not a replacement primary result.
  - P6: SUPERSEDES P2 to P5 as of 2026-09-04. Study v2 ran on the batched-block harness and the frozen gate PASSES all four checks, including `pre_onset_negative_controls_include_zero`. Rewrite P2 and P3 against v2 numbers: stand_motor +0.000 pp [+0.000, +0.000]; stand_obs +0.000 pp [+0.000, +0.000]; walk_motor +0.065 pp [+0.000, +0.195]; walk_obs +0.065 pp [+0.000, +0.195]. State explicitly that the two walking cells pass by touching zero, not by straddling it. The remaining residual is a post-reset onset-observation divergence, now POSITIVELY MEASURED rather than inferred by elimination: a single onset-tick threshold separates divergent from bit-identical blocks in 12 of 12 arm pairs, which identifies within-tick coupling in the shared physics batch and rules out temporal carryover. Draft prose, the full number set, the contamination-free sensitivity table, and the reproduction command are in `paper/onset_residual_limitation.md`. Do not rewrite those numbers by hand; they come from `reliability_eval/causal_viability_replication_v2/onset_residual_audit.json`.
- Figures/tables placed here:
  - Table III: pre-onset negative control, all four cells plus family pooling. Supports N1. This table is non-negotiable; the control is reported as a table, not as a sentence in a limitations list.
...
- BLOCKER: as of 2026-08-24 the study code (`scripts/reliability_closed_loop.py`, `src/phoenix/reliability/study.py`, the whole `reliability_eval/causal_viability_replication/` tree) is uncommitted per vault `tasks.md`. A repo link is not a valid appendix until that lands. Scheduled Day 2.
...
| 6 Threats to Validity | 450 | Table III (0.22) | 0.65 |
| 7 Limitations | 250 | none | 0.24 |
| 8 Conclusion | 120 | none | 0.11 |
| Body total | 4,500 | 2.07 pages of assets | 6.28 body, plus about 1.0 for references = 7.3 of 8 |

Over budget by about 0.28 page. Pre-committed cut order, so the cut is a decision made now rather than a panic on Day 20:
...
3. If still over: cut the appendix entirely and point at the repo (already the fallback, since the code is uncommitted).
4. Never cut: Table III, Section 6, or Section 7. The honest-reporting sections are load-bearing for the paper's credibility and for the contract.
