...
| **R4** | **`base_lin_vel` is fed as zeros at deploy but is real in training.** The policy was trained with true body linear velocity in obs dims 0..2; the robot has no estimator, so the node feeds `np.zeros(3)`. | `observation.py:223` `resolve_base_lin_vel`, selected by the required `observation.base_lin_vel_source` key | **UPDATED 2026-09-12.** There is no longer a silent zero to reproduce: the source is an explicit, required config choice with no default, and every shipped config selects `zeros`. Port the *selector*, not the constant. The original warning still holds in full and is the reason `zeros` remains the shipped value: switching to `odom` changes the input distribution the policy was trained and calibrated in and invalidates the shield artifact, so it is a research decision, not a port decision. A port must refuse to start if the key is absent rather than assuming either source. |
| **R5** | **`default_joint_pos` must equal the training asset's default pose. FIXED 2026-09-11 in `72d0457`; all six configs now carry the training pose and `tests/test_deploy_default_joint_pos.py` pins it. Kept here because the C++ requirement is unchanged.** Isaac Lab's `use_default_offset=True` makes it both the action offset and the `joint_pos_rel` reference. Training uses hips L=+0.1 / R=−0.1. | `deploy_stand_h25.yaml:40-49` (correct) vs `deploy.yaml:31-34` etc. (all-zero hips) | Refuse to run on a config whose `default_joint_pos` does not match a value carried in the model/artifact provenance. Do not silently accept a plausible-looking pose. |
