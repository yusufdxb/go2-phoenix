...
if [[ $MOCK -eq 1 ]]; then
    c_yellow "[mock] skipping T7 rsync (no T7 mount on the dev workstation for self-test)"
else
    if [[ ! -d "$T7_MOUNT" ]]; then
...
if [[ $MOCK -eq 1 ]]; then
    c_yellow "[mock] skipping git ff-only (would mutate local repo on the dev workstation)"
else
    cd "$JETSON_REPO"
