...
def verdict(cfg, name, m):
    return gate_verdict(
        {
            "spec_checks": [],
            "scenarios": {name: {"checks": evaluate_scenario(m, cfg.scenario(name), cfg)}},
        }
    )
...
def test_v3_is_default_and_equals_v2_plus_responsiveness():
    ...
    assert resp == {
        "min_fraction": 0.5,
        "scenarios": {
            "forward_0p3": "lin_vel_x",
            "forward_0p5": "lin_vel_x",
            "yaw_0p6": "ang_vel_z",
        },
    }
...
@pytest.mark.parametrize(
    "name,key,cmd",
    [
        ("forward_0p3", "tracked_mean_vx", 0.3),
...
def test_frozen_policy_fails_and_responsive_policy_passes(name, key, cmd):
    ...
    assert frozen["failures"] == [
        f"{name}:responsiveness.{'ang_vel_z' if 'wz' in key else 'lin_vel_x'}"
    ]
    assert (
        verdict(cfg, name, metrics(**{key: 0.5 * cmd}))["verdict"] == "PASS"
    )  # boundary inclusive
...
def test_responsiveness_is_safety_tier_and_fall_fails_it():
    ...
    checks = evaluate_scenario(
        metrics(fell=True, tracked_mean_vx=None), cfg.scenario("forward_0p3"), cfg
    )
...
def test_trace_hook_records_every_control_step_without_changing_metrics():
    ...
    m = build_manifest(
        ...
        commands=CommandRanges((-1, 1), (-0.5, 0.5), (-1, 1), 0.1),
        git_sha="a" * 40,
        git_dirty=False,
        seed=1,
        task="t",
    ...
    tr = []
    a = run_gate_scenario(
        spec, lambda o: np.zeros(12), s, cfg, RunOptions(latency_ms=0.0, trace=tr)
    )["metrics"]
    b = run_gate_scenario(spec, lambda o: np.zeros(12), s, cfg, RunOptions(latency_ms=0.0))[
        "metrics"
    ]
